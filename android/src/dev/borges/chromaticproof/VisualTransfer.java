...
public final class VisualTransfer implements AutoCloseable {
    ...
    public VisualTransfer(File directory) throws IOException {
        ...
        this.partFile = File.createTempFile("chromatic-visual-", ".part", directory);
        this.part = new RandomAccessFile(partFile, "rw");
        this.part.setLength(0);
        this.received = new BitSet(VisualFrame.PAYLOAD_MAX);
    }
    ...
    public boolean accept(VisualFrame frame) throws IOException {
        ...
        if (deferred != null) {
            throw new IOException("transfer failed earlier: " + deferred.getMessage(), deferred);
        }
        try {
            if (frame.isManifest()) {
    ...
    private boolean acceptManifest(VisualFrame frame) throws IOException {
        ...
        if (n.equals(name) && frame.fileSize == fileSize && frame.fileCrc == fileCrc) {
            return false; // identical manifest: ignore
        }
        throw new IOException("conflicting manifest: latched \"" + name
                + "\" crc=" + Long.toHexString(fileCrc) + " size=" + fileSize
                + ", got \"" + n + "\" crc=" + Long.toHexString(frame.fileCrc)
                + " size=" + frame.fileSize);
    }
    ...
    private boolean acceptData(VisualFrame frame) throws IOException {
        ...
        long off = (long) block * VisualFrame.PAYLOAD_MAX;
    ...
    private void compareDuplicate(VisualFrame frame, int block, long off) throws IOException {
    ...
    private boolean maybeComplete() throws IOException {
        ...
        if (len != fileSize) {
            throw new IOException("staged length " + len + " != manifest " + fileSize);
        }
        CRC32 crc = new CRC32();
        ...
        if (crc.getValue() != fileCrc) {
            throw new IOException("whole-file CRC mismatch: expected "
                    + Long.toHexString(fileCrc) + ", staged " + Long.toHexString(crc.getValue()));
        }
        promote();
        return true;
    }
    ...
    private static int blocksFor(long size) {
        return (int) ((size + VisualFrame.PAYLOAD_MAX - 1) / VisualFrame.PAYLOAD_MAX);
    }
