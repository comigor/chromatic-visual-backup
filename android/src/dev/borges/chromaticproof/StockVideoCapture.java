 ...
 * video interface and delivers the luma plane of every intact frame to a
...
public final class StockVideoCapture implements AutoCloseable {
    ...
    static {
        System.loadLibrary("chromatic_uvc");
    }

    /** Receives one luma plane; invoked on the thread that called run. */
    public interface Listener {
        void frame(byte[] luminance, int width, int height);
    }
    ...
    public static UsbDevice findDevice(UsbManager manager) {
        for (UsbDevice device : manager.getDeviceList().values()) {
            if (device.getVendorId() == VENDOR_ID_CHROMATIC && hasVideoInterface(device)) {
                return device;
            }
        }
        return null;
     ...
     */
    public StockVideoCapture(UsbManager manager, UsbDevice device) throws IOException {
        if (device.getVendorId() != VENDOR_ID_CHROMATIC || !hasVideoInterface(device)) {
            throw new IOException("Device is not a supported Chromatic video device.");
        }
        UsbDeviceConnection opened = manager.openDevice(device);
        if (opened == null) {
            throw new IOException("Opening the USB device failed (missing USB permission?).");
        }
        long nativeHandle;
        try {
    ...
    private static native void nRun(long handle, Object listener) throws IOException;
